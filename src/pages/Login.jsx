  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', paddingTop: '120px', backgroundColor: '#0B0F19', color: '#FFFFFF' }}>
      <div style={{ borderRadius: '12px', padding: '2.5rem', width: '100%', maxWidth: '400px', backgroundColor: '#1a1f2e', border: '1px solid #2a3142' }}>
        <h1 style={{ fontSize: '2rem', fontWeight: '700', textAlign: 'center', marginBottom: '2rem' }}>Welcome Back!</h1>
        <input
          type="email"
          placeholder="Email"
          value={email}
        ...
        <input
          ...
          onBlur={(e) => (e.target.style.borderColor = '#2a3142')}
        />
        <AnimatedButton onClick={() => alert('Logged in!')} style={{ width: '100%', padding: '0.75rem 1rem', fontSize: '1rem' }}>
          Login
        </AnimatedButton>
        <p style={{ textAlign: 'center', marginTop: '1.5rem', color: '#9CA3AF' }}>
          Don't have an account? <a href="/signup" style={{ color: '#9CA3AF', textDecoration: 'none' }} className="hover:text-white">Sign up</a>
        </p>
      </div>
    </div>
  );
